...
class ServiceGRPOPosting(models.Model):
    """
    Tracks service GRPO postings for transport bookings.
    These are SAP service-type Purchase Delivery Notes linked to dispatch plans.
    """

    dispatch_plan = models.ForeignKey(
        DispatchPlan,
        on_delete=models.PROTECT,
        related_name="service_grpo_postings",
    )
    ...
    class Meta:
        ordering = ["-created_at"]
        indexes = [
            models.Index(fields=["dispatch_plan", "status"]),
            models.Index(fields=["vendor_code", "status"]),
        ]

    def __str__(self):
        bill_no = self.dispatch_plan.sap_invoice_doc_num or self.dispatch_plan.sap_invoice_doc_entry
        return f"Service GRPO for dispatch bill {bill_no} - {self.status}"


class ServiceGRPOLinePosting(models.Model):
    """Tracks service lines posted in a transport service GRPO."""
...
class ServiceGRPOAttachment(models.Model):
    ...
    uploaded_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="service_grpo_attachments",
    )
