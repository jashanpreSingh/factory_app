...
from .serializers import (
    ...
    DispatchGateLockSerializer,
    DispatchGateLockUpdateSerializer,
    DispatchGateOutActionSerializer,
    DispatchGateOutAttachmentSerializer,
    DispatchGateOutCreateSerializer,
    DispatchGateOutSerializer,
    DispatchGateOutUpdateSerializer,
    parse_dispatch_line_decimal,
...
def get_dispatch_gate_lock(company):
    lock, _ = DispatchGateLock.objects.get_or_create(company=company)
    return lock


def dispatch_gate_locked_response(lock):
    return Response(
        {
            "detail": lock.reason or "Sales dispatch gate-out is currently locked.",
            "locked": True,
            "reason": lock.reason,
        },
...
def dispatch_gate_out_queryset(company):
    return (
...
def apply_dispatch_plan_snapshot(plan, gate_out, user, booking_status=None):
    plan.sap_invoice_doc_num = gate_out.sap_invoice_doc_num or plan.sap_invoice_doc_num
    plan.vehicle = gate_out.vehicle
    plan.driver = gate_out.driver
    plan.linked_vehicle_entry = gate_out.vehicle_entry
    plan.dispatch_date = gate_out.gate_out_date
    plan.vehicle_no = gate_out.vehicle.vehicle_number
    plan.driver_name = gate_out.driver.name
    plan.driver_mobile_no = gate_out.driver.mobile_no or ""
    plan.kanta_weight = gate_out.net_weight
    if gate_out.vehicle.transporter:
        plan.transporter = gate_out.vehicle.transporter
        plan.transporter_name = gate_out.vehicle.transporter.name
    if booking_status:
        plan.booking_status = booking_status
    plan.updated_by = user
    plan.save()


def sync_dispatch_gate_lines(gate_out, raw_items, user):
    gate_out.items.all().delete()

    for index, item in enumerate(raw_items or [], start=1):
        line_num = item.get("line_num", index)
        DispatchGateOutLine.objects.create(
            entry=gate_out,
            line_num=int(line_num or index),
            item_code=item.get("item_code", "") or "",
            item_name=item.get("item_name", "") or "",
            order_qty=parse_dispatch_line_decimal(item.get("order_qty")),
            dispatched_qty=parse_dispatch_line_decimal(item.get("dispatched_qty")),
            uom=item.get("uom", "") or "",
...
def dispatch_entry_editable_response(gate_out):
    if gate_out.status != DispatchGateOutStatus.IN_PROGRESS:
        return Response(
            {"detail": "Only in-progress sales dispatch entries can be edited."},
            status=status.HTTP_400_BAD_REQUEST,
        )
    if gate_out.print_commit:
        return Response(
            {"detail": "This dispatch gate pass has already been committed."},
            status=status.HTTP_400_BAD_REQUEST,
        )
    return None


class DispatchGateLockView(APIView):
    permission_classes = [IsAuthenticated, HasCompanyContext]

    def get(self, request):
        lock = get_dispatch_gate_lock(request.company.company)
        return Response(DispatchGateLockSerializer(lock).data)

    def patch(self, request):
        serializer = DispatchGateLockUpdateSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data

        lock = get_dispatch_gate_lock(request.company.company)
        lock.locked = data["locked"]
        lock.reason = data.get("reason", "")
...
class DispatchGateOutListCreateView(APIView):
    permission_classes = [IsAuthenticated, HasCompanyContext]

    def get(self, request):
        qs = dispatch_gate_out_queryset(request.company.company)
        ...
        return Response(DispatchGateOutSerializer(qs, many=True).data)

    def post(self, request):
        lock = get_dispatch_gate_lock(request.company.company)
        if lock.locked:
            return dispatch_gate_locked_response(lock)

        serializer = DispatchGateOutCreateSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data
        ...
        duplicate = (
            ...
            .filter(
                company=request.company.company,
                sap_invoice_doc_entry=data["sap_invoice_doc_entry"],
                is_active=True,
            )
            .exclude(status__in=[
                DispatchGateOutStatus.CANCELLED,
                DispatchGateOutStatus.REJECTED,
            ])
            .order_by("-created_at")
            .first()
        )
            ...
            return Response(
                {
                    "detail": (
                        ...
                        f"Sales Dispatch entry {duplicate.entry_no}."
                    ),
                    "linked_dispatch_gate_out_id": duplicate.id,
        ...
        if data.get("linked_vehicle_entry_id"):
            linked_vehicle_entry = get_object_or_404(
                VehicleEntry,
                id=data["linked_vehicle_entry_id"],
                company=request.company.company,
            )

        with transaction.atomic():
            plan, _ = DispatchPlan.objects.get_or_create(
                company=request.company.company,
                sap_invoice_doc_entry=data["sap_invoice_doc_entry"],
                defaults={
                    "sap_invoice_doc_num": data.get("sap_invoice_doc_num", ""),
                    "created_by": request.user,
                    "updated_by": request.user,
                },
            )
            gate_out = DispatchGateOut.objects.create(
                company=request.company.company,
                entry_no=DispatchGateOut.generate_entry_no(),
                dispatch_plan=plan,
                ...
                sap_weight=data.get("sap_weight"),
                gate_out_date=data["gate_out_date"],
                out_time=data["out_time"],
                security_name=data.get("security_name", ""),
                seal_no=data.get("seal_no", ""),
                pgi_document_no=data.get("pgi_document_no", ""),
                goods_issue_posted=data.get("goods_issue_posted", False),
                invoice_checked=data.get("invoice_checked", False),
                delivery_note_checked=data.get("delivery_note_checked", False),
                eway_bill_checked=data.get("eway_bill_checked", False),
                lr_checked=data.get("lr_checked", False),
                physical_quantity=data["physical_quantity"],
                physical_uom=data["physical_uom"],
                remarks=data.get("remarks", ""),
                created_by=request.user,
                updated_by=request.user,
            )
            sync_dispatch_gate_lines(gate_out, data.get("items", []), request.user)
            apply_dispatch_plan_snapshot(
                plan,
                gate_out,
                request.user,
        ...
        return Response(
            DispatchGateOutSerializer(gate_out).data,
            status=status.HTTP_201_CREATED,
        )


class DispatchGateOutDetailView(APIView):
    permission_classes = [IsAuthenticated, HasCompanyContext]

    def get_object(self, request, entry_id):
        return get_object_or_404(
            dispatch_gate_out_queryset(request.company.company),
            id=entry_id,
        )

    def get(self, request, entry_id):
        gate_out = self.get_object(request, entry_id)
        return Response(DispatchGateOutSerializer(gate_out).data)

    def patch(self, request, entry_id):
        gate_out = self.get_object(request, entry_id)
        blocked = dispatch_entry_editable_response(gate_out)
        if blocked:
            return blocked

        serializer = DispatchGateOutUpdateSerializer(data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data

        with transaction.atomic():
            for field, value in data.items():
                setattr(gate_out, field, value)
            gate_out.updated_by = request.user
            gate_out.save()
            if gate_out.dispatch_plan_id:
                apply_dispatch_plan_snapshot(
                    gate_out.dispatch_plan,
                    gate_out,
                    request.user,
                    booking_status=DispatchPlanStatus.BOOKED,
                )
...
class DispatchGateOutAttachmentView(APIView):
    permission_classes = [IsAuthenticated, HasCompanyContext]
    ...
    def patch(self, request, entry_id):
        ...
        serializer = DispatchGateOutAttachmentSerializer(data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data

        for field in ["dock_photo", "gatepass_document", "attachment_notes", "remarks"]:
            if field in data:
                setattr(gate_out, field, data[field])
        gate_out.updated_by = request.user
        gate_out.save()
...
def dispatch_complete_validation_error(gate_out):
    missing = []
    if not gate_out.seal_no:
        missing.append("Seal number")
    if not gate_out.physical_quantity or gate_out.physical_quantity <= 0:
        missing.append("Physical quantity")
    if not gate_out.physical_uom:
        missing.append("Physical UOM")
    if not gate_out.goods_issue_posted:
        missing.append("Goods issue confirmation")
    if not gate_out.invoice_checked:
        missing.append("Invoice check")
    if not gate_out.delivery_note_checked:
        missing.append("Delivery note check")
    if not gate_out.eway_bill_checked:
        missing.append("E-way bill check")
    if not gate_out.lr_checked:
        missing.append("LR / freight document check")
    if gate_out.gross_weight is None or gate_out.gross_weight <= 0:
        missing.append("Gross weight")
    if gate_out.tare_weight is None or gate_out.tare_weight < 0:
        missing.append("Tare weight")
    if (
        gate_out.gross_weight is not None
        and gate_out.tare_weight is not None
        and gate_out.tare_weight > gate_out.gross_weight
    ):
        return "Tare weight cannot be greater than gross weight."
    if not gate_out.dock_photo:
        missing.append("Dock photo")
    if not gate_out.gatepass_document:
        missing.append("Gatepass document")

    if missing:
        return f"Complete the required fields first: {', '.join(missing)}."
    return ""


class DispatchGateOutCompleteView(APIView):
    permission_classes = [IsAuthenticated, HasCompanyContext]
    ...
    def post(self, request, entry_id):
        ...
        validation_error = dispatch_complete_validation_error(gate_out)
        if validation_error:
            return Response({"detail": validation_error}, status=status.HTTP_400_BAD_REQUEST)

        with transaction.atomic():
            gate_out.ensure_gatepass()
            gate_out.status = DispatchGateOutStatus.COMPLETED
            gate_out.updated_by = request.user
            gate_out.save()
            if gate_out.dispatch_plan_id:
                apply_dispatch_plan_snapshot(
...
class DispatchGateOutCommitPrintView(APIView):
    permission_classes = [IsAuthenticated, HasCompanyContext]
    ...
    def post(self, request, entry_id):
        ...
        if gate_out.status != DispatchGateOutStatus.COMPLETED:
            return Response(
                {"detail": "Complete the gate-out entry before committing the printed gate pass."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        with transaction.atomic():
            gate_out.ensure_gatepass()
            gate_out.print_commit = True
            gate_out.committed_at = timezone.now()
            gate_out.updated_by = request.user
            gate_out.save()
            if gate_out.dispatch_plan_id:
                apply_dispatch_plan_snapshot(
...
class DispatchGateOutCancelView(APIView):
    permission_classes = [IsAuthenticated, HasCompanyContext]

    def post(self, request, entry_id):
        serializer = DispatchGateOutActionSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        ...
        if gate_out.status != DispatchGateOutStatus.IN_PROGRESS or gate_out.print_commit:
            return Response(
                {"detail": "Only in-progress, uncommitted sales dispatch entries can be cancelled."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        with transaction.atomic():
            gate_out.status = DispatchGateOutStatus.CANCELLED
            gate_out.cancel_reason = serializer.validated_data.get("reason", "")
            gate_out.cancelled_at = timezone.now()
            gate_out.updated_by = request.user
            gate_out.save()
            if gate_out.dispatch_plan_id:
...
class DispatchGateOutRejectView(APIView):
    permission_classes = [IsAuthenticated, HasCompanyContext]
    ...
    def post(self, request, entry_id):
        ...
        if gate_out.print_commit:
            return Response(
                {"detail": "A committed printed gate pass cannot be rejected."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if gate_out.status in [DispatchGateOutStatus.CANCELLED, DispatchGateOutStatus.REJECTED]:
            return Response(
                {"detail": "This sales dispatch entry is already closed."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        with transaction.atomic():
            gate_out.status = DispatchGateOutStatus.REJECTED
            gate_out.rejected_reason = serializer.validated_data.get("reason", "")
            gate_out.rejected_at = timezone.now()
            gate_out.updated_by = request.user
            gate_out.save()
            if gate_out.dispatch_plan_id:
                apply_dispatch_plan_snapshot(
                    ...
                    gate_out,
                    request.user,
                    booking_status=DispatchPlanStatus.CANCELLED,
                )
